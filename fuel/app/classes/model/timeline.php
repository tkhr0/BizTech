...
class Model_Timeline extends \Model {
  public static function get_containers($user_id, $limit){
    $TYPE = 2; //TODO
    $containers = \DB::query('SELECT goals.user_id as user_id, goals.name as goal_name, goals.achieve as achieve, goals.active as active, goals.cheered as goal_cheered, containers.id as container_id, containers.cheered as cheer_num, containers.created_at as created_at, containers.status as status FROM follows JOIN goals JOIN containers ON follows.to_user_id=goals.user_id AND goals.id=containers.goal_id WHERE follows.from_user_id=' . $user_id . " ORDER BY created_at LIMIT " . $limit)->execute()->as_array();
